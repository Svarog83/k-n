...
/**
 * Class for showing basic form elements
 */
class FormHelperBasic
{
	private $options = array( );
	private $ACUsedNames = array( );
	...
	public function showTextArea( $field_name, $value, $arr = array( ) )
	{
		return '<textarea
            ...
            ' .
		       ...
		       ( isset ( $arr['validation'] ) && $arr['validation'] ? ' class="' . $arr['validation'] . '"' : '' ) .
		       ( isset ( $arr['minlength'] ) && $arr['minlength'] ? ' minlength="' . $arr['minlength'] . '"' : '' ) .
		       ( isset ( $arr['onchange'] ) && $arr['onchange'] ? ' onChange="' . $arr['onchange'] . '"' : '' ) .
		       ( isset ( $arr['onfocus'] ) && $arr['onfocus'] ? ' onFocus="' . $arr['onfocus'] . '"' : '' ) .
		       ( isset ( $arr['onblur'] ) && $arr['onblur'] ? ' onBlur="' . $arr['onblur'] . '"' : '' ) .
		       ( isset ( $arr['add_str'] ) && $arr['add_str'] ? $arr['add_str'] : '' ) . '>' . $value . '</textarea>' .
		       ( isset ( $arr['add_html'] ) && $arr['add_html'] ? $arr['add_html'] : '' ) . '';
	}
	...
	public function showCheckBox( $field_name, $value, $arr )
	{
		...
		return '
            <input type="checkbox" name="' . $field_name . '" value="' . ( !empty ( $value ) ? $value : 1 ) . '" ' .
		       ( isset ( $arr['id'] ) && $arr['id'] ? ' id="' . $arr['id'] . '"' : '' ) .
		       ( isset ( $arr['validation'] ) && $arr['validation'] ? ' class="' . $arr['validation'] . '"' : '' ) .
		       ( isset ( $arr['checked'] ) && $arr['checked'] ? ' checked="checked"' : '' ) .
		       ( isset ( $arr['onchange'] ) && $arr['onchange'] ? ' onChange="' . $arr['onchange'] . '"' : '' ) .
		       ( isset ( $arr['onfocus'] ) && $arr['onfocus'] ? ' onFocus="' . $arr['onfocus'] . '"' : '' ) .
		       ( isset ( $arr['onblur'] ) && $arr['onblur'] ? ' onBlur="' . $arr['onblur'] . '"' : '' ) .
		       ( isset ( $arr['add_str'] ) && $arr['add_str'] ? $arr['add_str'] : '' ) . '>' .
	...
	public function showDate( $field_name, $value, $arr = array( ) )
	{
		return '<span><input
            ...
            name="' . $field_name . '"
            readonly
            class="datepicker' . ( isset( $arr['currtime'] ) ? ' currtime' : '' ) . ( isset ( $arr['validation'] ) && $arr['validation'] ? ' ' . $arr['validation'] : '' ) . '"
            size="' . ( !empty( $arr['size'] ) ? $arr['size'] : '10' ) . '"
            value="' . ( $value != '0000-00-00' ? Func::formatDate( $value, ( isset ( $arr['format'] ) ? $arr['format'] : 'yy-mm-dd' ) ) : '' ) . '"
            ' . ( !empty( $arr['onchange'] ) ? ' onChange="' . $arr['onchange'] . '"' : '' ) .
		       ( isset ( $arr['id'] ) && $arr['id'] ? ' id="' . $arr['id'] . '"' : '' ) .
		       ( isset ( $arr['equalTo'] ) && $arr['equalTo'] ? ' equalTo="' . $arr['equalTo'] . '"' : '' ) .
		       ( isset ( $arr['minlength'] ) && $arr['minlength'] ? ' minlength="' . $arr['minlength'] . '"' : '' ) .
		       ...
		       ( isset ( $arr['onblur'] ) && $arr['onblur'] ? ' onBlur="' . $arr['onblur'] . '"' : '' ) .
		       ( isset ( $arr['add_str'] ) && $arr['add_str'] ? $arr['add_str'] : '' ) . '>' .
		       ( isset ( $arr['add_html'] ) && $arr['add_html'] ? $arr['add_html'] : '' ) . ( empty( $arr['noclean'] ) ?
            ...
            ( isset ( $arr['format'] ) && $arr['format'] ? $arr['format'] : 'yy-mm-dd' ) .
...
?>
	...
	<script language="JavaScript">
		...
		$j( function()
		{
			...
			$j( "#<?= $options['id']?>" ).OMSAutocomplete( obj_settings );
			gAC_ID = <?= $included ?>;
			...
			<? if ( !in_array( $ac_name, $this->ACUsedNames ) ): ?>
	...
	<img src="/icon/edit_empty.gif" width="16" height="16" border="0" title="__**Clean field**__" alt="__**Clean**__"
	     style="cursor:pointer;" OnClick="ACFieldClear(this);">
	<? endif; ?>

	<?
		$this->ACUsedNames[] = $ac_name;
	...
	public function showInline( $title, $field_name, $value, $arr = array( ) )
	{
		...
		return $this->showTextAutoComplete( $ac_name, $arr['add_name'],
		                                        array(
		                                             'size' => ( isset ( $arr['size'] ) ? $arr['size'] : 20 ),
		                                             'maxLength' => ( isset ( $arr['maxlength'] ) ? $arr['maxLength']
				                                             : 30 ),
		                                             'ac_name' => $ac_name,
		                                             'title' => $title,
